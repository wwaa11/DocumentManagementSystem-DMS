<form id="process-form" action="{{ route('admin.purchase.process') }}" method="post" enctype="multipart/form-data">
    @csrf
...
@push('scripts')
    <script>
        ...
        async function cancelJob() {
            ...
            if (swal.isConfirmed) {
                axios.post('{{ route('admin.purchase.canceljob') }}', {
                    id: {{ $document->id }},
                    ...
                    if (response.data.status == 'success') {
                        Swal.fire({
                            ...
                            window.location.href = '{{ route('admin.purchase.mylist') }}';
                        });
                    } else {
                        Swal.fire({
        ...
        async function cancelDocument() {
            ...
            if (swal.isConfirmed && swal.value) {
                axios.post('{{ route('admin.purchase.cancel') }}', {
                    id: {{ $document->id }},
                    reason: swal.value,
                }).then(function(response) {
                    if (response.data.status == 'success') {
