<form id="process-form" action="{{ route("admin.user.process") }}" method="post" enctype="multipart/form-data">
    @csrf
...
@push("scripts")
    ...
    <script>
        ...
        async function cancelJob() {
            ...
            if (swal.isConfirmed) {
                axios.post("{{ route("admin.user.canceljob") }}", {
                    id: {{ $document->id }},
                    type: '{{ $type }}',
                    ...
                    if (response.data.status == "success") {
                        Swal.fire({
                            ...
                            window.location.href = "{{ route("admin.user.mylist", $type) }}";
                        });
                    } else {
                        Swal.fire({
        ...
        async function cancelDocument() {
            ...
            if (swal.isConfirmed && swal.value) {
                axios.post("{{ route("admin.user.cancel") }}", {
                    id: {{ $document->id }},
                    type: '{{ $type }}',
                    reason: swal.value,
                }).then(function(response) {
