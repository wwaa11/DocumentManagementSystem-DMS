...
@section("content")
    <div class="mx-8 pb-10">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-primary text-3xl font-bold">User Service Report</h1>
                <p class="text-base-content/60 text-sm">Dashboard สรุปข้อมูลการให้บริการสิทธิใช้งานระบบ (PAC, LAB, HeartStream, Register)</p>
            </div>
            <div class="bg-base-100 rounded-lg p-4 shadow-md">
                <form class="flex items-end gap-3" action="{{ route("admin.user.reportlist") }}" method="GET">
                    ...
                    <a class="btn btn-ghost btn-sm border-base-content/20" href="{{ route("admin.user.reportlist", ["type" => $type]) }}">
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
            @foreach (["pac" => "PACS", "lab" => "LAB", "heartstream" => "HeartStream", "register" => "Registration"] as $k => $label)
                @if (!$type || $type == "ALL" || $type == $k)
                    <div class="card bg-base-100 border-base-200 border shadow-sm">
                        <div class="card-body p-4">
                            <h3 class="flex items-center truncate text-sm font-bold">
                                <i class="fas fa-lock text-primary mr-2"></i> {{ $label }}
                            </h3>
                            <div class="divider my-1"></div>
                            <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                                <div class="flex justify-between"><span>Wait Approval:</span> <span class="text-info font-bold">{{ $statsByType[$k]["wait_approval"] }}</span></div>
                                <div class="flex justify-between"><span>Pending:</span> <span class="text-warning font-bold">{{ $statsByType[$k]["pending"] }}</span></div>
                                <div class="flex justify-between"><span>Processing:</span> <span class="text-primary font-bold">{{ $statsByType[$k]["process"] }}</span></div>
                                <div class="flex justify-between"><span>Done:</span> <span class="text-secondary font-bold">{{ $statsByType[$k]["done"] }}</span></div>
                                <div class="flex justify-between"><span>Completed:</span> <span class="text-success font-bold">{{ $statsByType[$k]["complete"] }}</span></div>
                                <div class="flex justify-between"><span>Rejected:</span> <span class="text-error font-bold">{{ $statsByType[$k]["reject"] }}</span></div>
                                <div class="col-span-2 mt-1 flex justify-between border-t pt-1 font-bold"><span>Total:</span> <span>{{ $statsByType[$k]["total"] }}</span></div>
                            </div>
                        </div>
                    </div>
                @endif
            ...
            <div class="card bg-base-100 shadow-xl">
                <div class="card-body">
                    <h2 class="card-title mb-4 flex items-center">
                        ...
                        ประสิทธิภาพเจ้าหน้าที่ ({{ strtoupper($type ?? "ALL") }})
                    </h2>
                    <div class="h-80 overflow-y-auto">
                        <table class="table-zebra table">
                            <thead class="bg-base-100 sticky top-0 italic">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Chart(deptCtx, {
                type: 'bar',
                data: {
                    labels: deptLabels.slice(0, 10),
                    datasets: [{
                        label: 'จำนวนการแจ้งเอกสาร',
