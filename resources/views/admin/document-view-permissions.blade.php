...
@section('content')
    <div class="mx-8 pb-10">
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <h1 class="text-primary text-3xl font-bold">Department Document Access</h1>
                <p class="text-base-content/60 text-sm">กำหนดผู้ที่สามารถดูเอกสารทั้งหมดจากแผนกที่เลือก ในหน้าเอกสารของฉัน</p>
            </div>
            <div class="bg-base-100 border-base-200 rounded-lg border p-3 shadow-sm">
                <form class="flex items-center gap-2" action="{{ route('admin.document-view-permissions') }}" method="GET">
                    <div class="join">
                        <input
                    ...
                    @if ($search)
                        <a class="btn btn-ghost btn-sm btn-circle" href="{{ route('admin.document-view-permissions') }}">
                            <i class="fas fa-times"></i>
                        </a>
                    @endif
                </form>
                <p class="text-base-content/50 mt-2 text-xs">ค่าเริ่มต้นแสดงเฉพาะผู้ที่มีสิทธิ์ดูเอกสารแผนกแล้ว</p>
            </div>
        </div>
        ...
        <div class="space-y-4" id="document-view-permission-list">
            @forelse ($users as $user)
                @php
                    $selectedDepartments = $user->viewDepartments();
                @endphp
                <form
                    class="document-view-permission-form card bg-base-100 border-base-200 border shadow-xl"
                    action="{{ route('admin.document-view-permissions.update') }}"
                    method="POST"
                    ...
                    <div class="card-body gap-5 p-6">
                        <div class="flex flex-wrap items-start justify-between gap-4">
                            ...
                            <label class="label cursor-pointer justify-start gap-3 rounded-xl border border-base-200 px-4 py-3">
                                <input
                                    class="toggle toggle-primary view-can-access"
                                    type="checkbox"
                                    name="can_view_department_documents"
                                    value="1"
                                    {{ $user->can_view_department_documents ? 'checked' : '' }}
                                >
                                <span class="label-text font-medium">ดูเอกสารแผนกได้</span>
                            </label>
                        </div>

                        <div class="department-picker {{ $user->can_view_department_documents ? '' : 'pointer-events-none opacity-50' }}">
                            <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
                                <div class="text-sm font-bold">แผนกที่ดูเอกสารได้</div>
                            ...
                            <div class="join mb-3 w-full max-w-xl">
                                <input
                                    ...
                                    {{ $user->can_view_department_documents ? '' : 'disabled' }}
                                >
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('.document-view-permission-form').forEach((form) => {
                const canView = form.querySelector('.view-can-access');
                ...
                canView.addEventListener('change', syncEnabled);
