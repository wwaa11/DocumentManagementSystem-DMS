                    ...
                    @foreach ($documents as $document)
                        ...
                        <tr class="hover:bg-base-300 {{ $isNewJobOverdue ? 'bg-error/10' : '' }}">
                            ...
                            <td class="text-xs">
                                ...
                                <div>{{ $document->document_type_name }}</div>
                            </td>
                            <td class="text-xs">
                            ...
                            <td class="text-center">
                                <div class="flex flex-wrap items-center justify-center gap-2">
                                    ...
                                    @endif
                                    <a class="btn btn-outline btn-accent" href="{{ route('admin.media.view', ['document_id' => $document->id, 'action' => $action]) }}">ดูเอกสาร</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
        ...
        <script>
            function approveAllDocuments() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.post('{{ route('admin.media.completeall') }}').then((response) => {
                            if (response.data.status == 'success') {
                                Swal.fire({
                                    title: 'สำเร็จ',
                                    text: response.data.message,
