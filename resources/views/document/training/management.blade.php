@if ($document->tasks->where("task_user", auth()->user()->userid)->count() > 0 && $document->status == "pending")
    <button class="btn btn-error btn-lg shadow-error/20 m-6 w-full gap-2 rounded-full p-6 px-10 shadow-lg md:w-auto" onclick="cancelTraining()">
    ...
    <div class="card bg-base-100 border-base-200 mb-8 mt-6 overflow-hidden border shadow-xl">
        @if ($document->training_id == null)
            ...
            <div class="flex flex-col items-center gap-6 p-6 md:flex-row">
                ...
                <button class="btn btn-success btn-lg shadow-accent/20 w-full gap-2 rounded-full px-10 shadow-lg transition-all hover:scale-105 active:scale-95 md:w-auto" onclick="createTraining()">
        ...
        @else
            ...
            <div class="text-error flex flex-col gap-2 p-6 text-xs">
